...
xsav <- getCodebook(file.path(tmp, "x.sav"))

convert(x, to = file.path(tmp, "x.xml"))
xxml <- getCodebook(file.path(tmp, "x.xml"))
rxml <- readLines(file.path(tmp, "x.xml"))

convert(x, to = file.path(tmp, "x.dta"))
xdta <- getCodebook(file.path(tmp, "x.dta"))

convert(x, to = file.path(tmp, "x.rds"))

xrds <- getCodebook(file.path(tmp, "x.rds"))


test_that("getCodebook() works", {
  ...
  # no metadata
  expect_error(getCodebook(data.frame(A = 1:5)))

  # still no metadata, but NULL instead of error
  expect_equal(getCodebook(data.frame(A = 1:5), error_null = TRUE), NULL)

  # file paths should be characters of length 1
  expect_error(getCodebook(c("file1", "file2")))
  expect_error(getCodebook(1))
})

test_that("an error is triggered if the XML file is not valid", {
    writeLines(rxml[-which(grepl("<docDscr>", rxml))], file.path(tmp, "x.xml"))
    expect_error(getCodebook(file.path(tmp, "x.xml")))
    expect_error(getCodebook(tmp, print_processing = FALSE))
})
...
#     writeLines(rxml, file.path(tmp, "x.xml"))
#     xdfm75 <- getCodebook(file.path(tmp, "x.xml"))
...
#     exportCodebook(xdfm, file = file.path(tmp, "x.xml"))
#     xxml <- getCodebook(file.path(tmp, "x.xml"))
